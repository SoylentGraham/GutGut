/*------------------------------------------------
  ...
  list of extension prototypes etc


-------------------------------------------------*/

#ifndef __GDISPLAYEXT__H_
#define __GDISPLAYEXT__H_
...
//	flags of hardware supported features
namespace GHardwareFlags
{
	const u32	MultiTexturing			= 1<<GHardware_MultiTexturing;			//	
	const u32	VertexBufferObjects		= 1<<GHardware_VertexBufferObjects;		//
	const u32	DrawRangeElements		= 1<<GHardware_DrawRangeElements;		//
	const u32	SwapInteral				= 1<<GHardware_SwapInterval;			//
	const u32	NVVertexProgram			= 1<<GHardware_NVVertexProgram;			//
	const u32	ARBVertexProgram		= 1<<GHardware_ARBVertexProgram;		//
	const u32	ARBFragmentProgram		= 1<<GHardware_ARBFragmentProgram;		//
}

extern const char* g_HardwareExtensionNames[GHardware_Max];
...
//	Types
//------------------------------------------------
typedef GList<void*>	GVoidPtrList;


//-------------------------------------------------------------------------
//	class wrapper for display extensions
//-------------------------------------------------------------------------
class GDisplayExt
{
public:
	u32					m_DisabledHardwareFlags;	//	manually disabled hardware features
	u32					m_HardwareFlags;			//	flags for what hardware features are supported
	GVoidPtrList		m_ExtensionAddresses[GHardware_Max];	//	list of function pointers for each hardware extension

public:
	GDisplayExt();

	Bool				Init();					//	initialises all extensions
	void				Shutdown();				//	shutdown all extensions
	Bool				AddExtensionFunction(const char* pFunctionName, int HardwareIndex );

	inline u32			HardwareSupportedFlags()				{	return (m_HardwareFlags);	};
	inline Bool			HardwareSupported(int HardwareIndex)	{	return ( HardwareSupportedFlags() & (1<<HardwareIndex) )!=0x0;	};

	inline u32			HardwareEnabledFlags()					{	return (m_HardwareFlags) & ~(m_DisabledHardwareFlags);	};
	inline Bool			HardwareEnabled(int HardwareIndex)		{	return ( HardwareEnabledFlags() & (1<<HardwareIndex) )!=0x0;	};

private:
	Bool				ExtensionSupported(const char* pExtensionName);

	
public: //	extensions
	#define DECLARE_EXT(TYPE, FUNC, HARDWARE, INDEX )	TYPE FUNC() {	return (TYPE)m_ExtensionAddresses[HARDWARE][INDEX];	}
